...
from detect_utils.utils 	import im2single, getWH, hsv_transform, IOU_centre_and_dims
from detect_utils.label_utils	import Label
from detect_utils.projection_utils import perspective_transform, find_T_matrix, getRectPts
...
def augment_sample(I,pts,dim,save_path_prefix):
	...
	llp = Label(0,tl,br)
	hsv_mod_str = "_".join("%0.2f"%x for x in hsv_mod.tolist())
	angles_str = "_".join("%0.2f"%x for x in angles.tolist())
	save_path_prefix = save_path_prefix+'-whratio_%0.2f_'%(whratio)+'hsv%s_'%hsv_mod_str+'flip%0.1f_'%(flip_prob)+'angle%s'%angles_str
	img_save_path = save_path_prefix+'.jpg'
	target_save_path = save_path_prefix
	if I.dtype == 'float32':
		import skimage.io
		Iroi = Iroi[...,(2,1,0)]
		skimage.io.imsave(img_save_path, Iroi)
	YY = labels2output_map(llp, pts, 208, 16)
	np.save(target_save_path, YY)
	return Iroi,llp,pts
