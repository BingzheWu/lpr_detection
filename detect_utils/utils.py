...
def image_files_from_folder(folder,upper=True):
	...
	return img_files
def label_files_from_folder(folder, upper = True):
	extensions = ['txt']
	label_files  = []
	for ext in extensions:
		label_files += glob('%s/*.%s' % (folder,ext))
		if upper:
			label_files += glob('%s/*.%s' % (folder,ext.upper()))
	return label_files
...
def make_optimizer(cfg, model):
    ...
    return optimizer
def save_checkpoint(state, dir_name, is_best = False, filename = 'checkpoint.pth.tar'):
    import torch
